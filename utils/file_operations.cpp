...
int get_init_data_from_file(const char *file_name, double *init_states) {
    char buffer_init[1023];
    FILE *fp_init;
    char *token;
    unsigned long idx = 0;
    unsigned int sample_size = 0;

    if ((fp_init = fopen(file_name, "r")) == NULL) {
        printf("Cannot open file %s\n", file_name);
        return 0;
    }

    fgets(buffer_init, sizeof(buffer_init), fp_init); // skip header

    while (fgets(buffer_init, sizeof(buffer_init), fp_init) != NULL) { // begin line reading
        token = strtok(buffer_init, ",");
        while (token != NULL) {
